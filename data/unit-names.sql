TRUNCATE `unit_real_names`;

INSERT INTO `unit_real_names` (uid, real_name) VALUES
('AAYLASECURA', "Aayla Secura"),
('ADMINISTRATORLANDO', "Administrator Lando"),
('ADMIRALACKBAR', "Admiral Ackbar"),
('ADMIRALPIETT', "Admiral Piett"),
('AHSOKATANO', "Ahsoka Tano"),
('AMILYNHOLDO', "Amilyn Holdo"),
('ANAKINKNIGHT', "Anakin Knight"),
('ARC170CLONESERGEANT', "Clone Sergeant's Ship"),
('ARC170REX', "Rex Clone Trooper"),
('ARCTROOPER501ST', "Arc Trooper"),
('ASAJVENTRESS', "Asaj Ventress"),
('AURRA_SING', "Aurra Sing"),
('B1BATTLEDROIDV2', "B1 Battle Droid V2"),
('B2SUPERBATTLEDROID', "B2 Superbattle Droid"),
('BARRISSOFFEE', "Barriss Offee"),
('BASTILASHAN', "Bastila Shan"),
('BASTILASHANDARK', "Bastila Shan Dark"),
('BAZEMALBUS', "Baze Malbus"),
('BB8', "Bb8"),
('BIGGSDARKLIGHTER', "Biggs Darklighter"),
('BISTAN', "Bistan"),
('BLADEOFDORIN', "Plo Koon's Blade Of Dorin"),
('BOBAFETT', "Boba Fett"),
('BODHIROOK', "Bodhi Rook"),
('BOSSK', "Bossk"),
('C3POCHEWBACCA', "Threepio & Chewie"),
('C3POLEGENDARY', "C-3PO"),
('CADBANE', "Cad Bane"),
('CANDEROUSORDO', "Canderous Ordo"),
('CAPITALCHIMAERA', "Capital Chimaera"),
('CAPITALJEDICRUISER', "Capital Jedi Cruiser"),
('CAPITALMALEVOLENCE', "Capital Malevolence"),
('CAPITALMONCALAMARICRUISER', "Capital Moncalamari Cruiser"),
('CAPITALNEGOTIATOR', "Capital Negotiator"),
('CAPITALRADDUS', "Capital Raddus"),
('CAPITALSTARDESTROYER', "Capital Star Destroyer"),
('CARTHONASI', "Carth Onasi"),
('CASSIANANDOR', "Cassian Andor"),
('CC2224', "CC-2224 (Cody)"),
('CHEWBACCALEGENDARY', "Chewbacca Legendary"),
('CHIEFCHIRPA', "Chief Chirpa"),
('CHIEFNEBIT', "Chief Nebit"),
('CHIRRUTIMWE', "Chirrut Imwe"),
('CHOPPERS3', "Chopper S3"),
('CLONESERGEANTPHASEI', "Clone Sergeant Phase I"),
('CLONEWARSCHEWBACCA', "Clone Wars Chewbacca"),
('COLONELSTARCK', "Colonel Starck"),
('COMMANDERLUKESKYWALKER', "Commander Luke Skywalker"),
('COMMANDSHUTTLE', "Command Shuttle"),
('CORUSCANTUNDERWORLDPOLICE', "Coruscant Underworld Police"),
('COUNTDOOKU', "Count Dooku"),
('CT210408', "CT-21-0408 (Echo)"),
('CT5555', "CT-5555 (Five)"),
('CT7567', "CT-7567 (Rex)"),
('DAKA', "Daka"),
('DARTHMALAK', "Darth Malak"),
('DARTHNIHILUS', "Darth Nihilus"),
('DARTHREVAN', "Darth Revan"),
('DARTHSIDIOUS', "Darth Sidious"),
('DARTHSION', "Darth Sion"),
('DARTHTRAYA', "Darth Traya"),
('DATHCHA', "Dathcha"),
('DEATHTROOPER', "Death Trooper"),
('DENGAR', "Dengar"),
('DIRECTORKRENNIC', "Director Krennic"),
('DROIDEKA', "Droideka"),
('EBONHAWK', "Ebon Hawk"),
('EETHKOTH', "Eeth Koth"),
('EMBO', "Embo"),
('EMPERORPALPATINE', "Emperor Palpatine"),
('EMPERORSSHUTTLE', "Emperors Shuttle"),
('ENFYSNEST', "Enfys Nest"),
('EPIXFINN', "Resistance Hero Finn"),
('EPIXPOE', "Resistance Hero Poe"),
('EWOKELDER', "Ewok Elder"),
('EWOKSCOUT', "Ewok Scout"),
('EZRABRIDGERS3', "Ezra Bridger S3"),
('FINN', "Finn"),
('FIRSTORDEREXECUTIONER', "First Order Executioner"),
('FIRSTORDEROFFICERMALE', "First Order Officer Male"),
('FIRSTORDERSPECIALFORCESPILOT', "First Order Special Forces Pilot"),
('FIRSTORDERTIEPILOT', "First Order Tie Pilot"),
('FIRSTORDERTROOPER', "First Order Trooper"),
('FOSITHTROOPER', "Fo Sith Trooper"),
('FULCRUMAHSOKA', "Fulcrum Ahsoka"),
('GAMORREANGUARD', "Gamorrean Guard"),
('GARSAXON', "Gar Saxon"),
('GAUNTLETSTARFIGHTER', "Gauntlet Starfighter"),
('GENERALHUX', "General Hux"),
('GENERALKENOBI', "General Kenobi"),
('GENERALSKYWALKER', "General Skywalker"),
('GEONOSIANBROODALPHA', "Geonosian Brood Alpha"),
('GEONOSIANSOLDIER', "Geonosian Soldier"),
('GEONOSIANSPY', "Geonosian Spy"),
('GEONOSIANSTARFIGHTER1', "Sun Fac's Starfighter"),
('GEONOSIANSTARFIGHTER2', "Geonosian Soldier's Starfighter"),
('GEONOSIANSTARFIGHTER3', "Geonosian Spy's Starfighter"),
('GHOST', "Ghost"),
('GLREY', "GL Rey"),
('GRANDADMIRALTHRAWN', "Grand Admiral Thrawn"),
('GRANDMASTERLUKE', "Jedi Master Luke"),
('GRANDMASTERYODA', "Grand Master Yoda"),
('GRANDMOFFTARKIN', "Grand Moff Tarkin"),
('GREEDO', "Greedo"),
('GREEFKARGA', "Greef Karga"),
('GRIEVOUS', "Grievous"),
('HANSOLO', "Han Solo"),
('HERASYNDULLAS3', "Hera Syndulla S3"),
('HERMITYODA', "Hermit Yoda"),
('HK47', "HK-47"),
('HOTHHAN', "Captain Han Solo"),
('HOTHLEIA', "Hoth Leia"),
('HOTHREBELSCOUT', "Hoth Rebel Scout"),
('HOTHREBELSOLDIER', "Hoth Rebel Soldier"),
('HOUNDSTOOTH', "Hounds Tooth"),
('HUMANTHUG', "Human Thug"),
('HYENABOMBER', "Hyena Bomber"),
('IG2000', "IG2000"),
('IG86SENTINELDROID', "IG86 Sentinel Droid"),
('IG88', "IG88"),
('IMAGUNDI', "Ima Gundi"),
('IMPERIALPROBEDROID', "Imperial Probe Droid"),
('IMPERIALSUPERCOMMANDO', "Imperial Super Commando"),
('JANGOFETT', "Jango Fett"),
('JAWA', "Jawa"),
('JAWAENGINEER', "Jawa Engineer"),
('JAWASCAVENGER', "Jawa Scavenger"),
('JEDIKNIGHTCONSULAR', "Jedi Knight Consular"),
('JEDIKNIGHTGUARDIAN', "Jedi Knight Guardian"),
('JEDIKNIGHTLUKE', "Jedi Knight Luke"),
('JEDIKNIGHTREVAN', "Jedi Knight Revan"),
('JEDISTARFIGHTERAHSOKATANO', "Jedi Starfighter Ahsoka Tano"),
('JEDISTARFIGHTERANAKIN', "Jedi Starfighter Anakin"),
('JEDISTARFIGHTERCONSULAR', "Jedi Starfighter Consular"),
('JOLEEBINDO', "Jolee Bindo"),
('JUHANI', "Juhani"),
('JYNERSO', "Jyn Erso"),
('K2SO', "K2SO"),
('KANANJARRUSS3', "Kanan Jarrus S3"),
('KITFISTO', "Kit Fisto"),
('KUIIL', "Kuiil"),
('KYLOREN', "Kylo Ren"),
('KYLORENUNMASKED', "Kylo Ren Unmasked"),
('L3_37', "L3 37"),
('LOBOT', "Lobot"),
('LOGRAY', "Logray"),
('LUKESKYWALKER', "Luke Skywalker"),
('LUMINARAUNDULI', "Luminara Unduli"),
('MACEWINDU', "Mace Windu"),
('MAGMATROOPER', "Magma Trooper"),
('MAGNAGUARD', "Magna Guard"),
('MAUL', "Maul"),
('MILLENNIUMFALCON', "Millennium Falcon"),
('MILLENNIUMFALCONEP7', "Millennium Falcon Ep7"),
('MILLENNIUMFALCONPRISTINE', "Millennium Falcon Pristine"),
('MISSIONVAO', "Mission Vao"),
('MONMOTHMA', "Mon Mothma"),
('MOTHERTALZIN', "Mother Talzin"),
('NIGHTSISTERACOLYTE', "Night Sister Acolyte"),
('NIGHTSISTERINITIATE', "Night Sister Initiate"),
('NIGHTSISTERSPIRIT', "Night Sister Spirit"),
('NIGHTSISTERZOMBIE', "Night Sister Zombie"),
('NUTEGUNRAY', "Nute Gunray"),
('OLDBENKENOBI', "Old Ben Kenobi"),
('PADMEAMIDALA', "Padme Amidala"),
('PAO', "Pao"),
('PAPLOO', "Paploo"),
('PHANTOM2', "Phantom 2"),
('PHASMA', "Captain Phasma"),
('PLOKOON', "Plo Koon"),
('POE', "Poe"),
('POGGLETHELESSER', "Poggle The Lesser"),
('PRINCESSLEIA', "Princess Leia"),
('QIRA', "Qi'ra"),
('QUIGONJINN', "Qui Gon Jinn"),
('R2D2_LEGENDARY', "R2D2 Legendary"),
('RANGETROOPER', "Range Trooper"),
('RESISTANCEPILOT', "Resistance Pilot"),
('RESISTANCETROOPER', "Resistance Trooper"),
('REY', "Rey"),
('REYJEDITRAINING', "Rey Jedi Training"),
('ROSETICO', "Rose Tico"),
('ROYALGUARD', "Royal Guard"),
('SABINEWRENS3', "Sabine Wren S3"),
('SAVAGEOPRESS', "Savage Opress"),
('SCARIFREBEL', "Scarif Rebel"),
('SHAAKTI', "Shaak Ti"),
('SHORETROOPER', "Shore Trooper"),
('SITHASSASSIN', "Sith Assassin"),
('SITHBOMBER', "Sith Bomber"),
('SITHPALPATINE', "Sith Eternal Emperor"),
('SITHFIGHTER', "Sith Fighter"),
('SITHINFILTRATOR', "Sith Infiltrator"),
('SITHMARAUDER', "Sith Marauder"),
('SITHTROOPER', "Sith Trooper"),
('SLAVE1', "Boba Fett's Slave 1"),
('SMUGGLERCHEWBACCA', "Smuggler Chewbacca"),
('SMUGGLERHAN', "Smuggler Han"),
('SNOWTROOPER', "Snow Trooper"),
('STORMTROOPER', "Storm Trooper"),
('STORMTROOPERHAN', "Storm Trooper Han"),
('SUNFAC', "Sun Fac"),
('SUPREMELEADERKYLOREN', "Supreme Leader Kylo Ren"),
('T3_M4', "T3 M4"),
('TALIA', "Talia"),
('TEEBO', "Teebo"),
('TIEADVANCED', "Tie Advanced"),
('TIEFIGHTERFIRSTORDER', "Tie Fighter First Order"),
('TIEFIGHTERFOSF', "Tie Fighter Fo Sf"),
('TIEFIGHTERIMPERIAL', "Tie Fighter Imperial"),
('TIEFIGHTERPILOT', "Tie Fighter Pilot"),
('TIEREAPER', "Tie Reaper"),
('TIESILENCER', "Tie Silencer"),
('THEMANDALORIAN', "The Mandalorian"),
('TUSKENRAIDER', "Tusken Raider"),
('TUSKENSHAMAN', "Tusken Shaman"),
('UGNAUGHT', "Ugnaught"),
('UMBARANSTARFIGHTER', "Umbaran Starfighter"),
('URORRURRR', "Urorrurrr"),
('UWINGROGUEONE', "Uwing Rogue One"),
('UWINGSCARIF', "Uwing Scarif"),
('VADER', "Vader"),
('VEERS', "Veers"),
('VISASMARR', "Visas Marr"),
('VULTUREDROID', "Vulture Droid"),
('WAMPA', "Wampa"),
('WATTAMBOR', "Wat Tambor"),
('WEDGEANTILLES', "Wedge Antilles"),
('WICKET', "Wicket"),
('XANADUBLOOD', "Xanadu Blood"),
('XWINGBLACKONE', "Poe Dameron's X-wing"),
('XWINGRED2', "Wedge Antilles's X-wing"),
('XWINGRED3', "Biggs Darklighter's X-wing"),
('XWINGRESISTANCE', "Resistance X-wing"),
('YOUNGCHEWBACCA', "Young Chewbacca"),
('YOUNGHAN', "Young Han"),
('YOUNGLANDO', "Young Lando"),
('YWINGCLONEWARS', "BLT-B Y-wing Starfighter"),
('YWINGREBEL', "Rebel Y-wing"),
('ZAALBAR', "Zaalbar"),
('ZAMWESELL', "Zam Wesell"),
('ZEBS3', "Garazeb Orrelios");