...
function registerGrandArenaResult(player, message, input_data) {
    initialValues = getInitializedGrandArenaValues(player.allycode);
 ...
 */
 ...
 exports.grandArenaRegistration = function(player, message) {
	...
	if (message.words.length < 7 && message.words.length != 0) {
		...
		let usage = "Usage: gar <type 5 or 3)> <win = 1, loss = 0> <opponent score> <ground territory cleared> <fleet territory cleared> <GL faced> <auto defense (0 or 1)>";
