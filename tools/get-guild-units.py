...
for member in guild['members']:
    ...
    if response.status_code != 200:
        print(response.text)
        sys.exit("Swgoh.gg player web service looks down for the moment")

    player = response.json()
    pName = player['data']['name'] if ally_code!='973486153' else 'PakPak'
    ...
    for unit in player['units']:
        ...
        level = "{:02d}".format(u['level'])
        relic = u['relic_tier']-2 if u['gear_level']>12 else 0
        line = [
            ally_code, str(u['combat_type']), str(u['level']), str(u['rarity']),
